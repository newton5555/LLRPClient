...
{
  [Serializable]
    public class LLRPClient<T> : IDisposable where T : CommunicationInterface, new()
  {
    ...
    private void TriggerRawReceived(byte[] raw)
    {
      try
      {
        if (this.OnRawReceived == null)
          return;
        this.OnRawReceived(raw);
      }
      catch
      {
      }
    ...
    private void TriggerRawSent(byte[] raw)
    {
      try
      {
