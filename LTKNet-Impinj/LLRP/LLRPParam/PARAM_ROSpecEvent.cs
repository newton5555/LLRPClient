...
namespace Org.LLRP.LTK.LLRPV1
{
  public class PARAM_ROSpecEvent : Parameter
  {
    ...
    public override string ToString()
    {
      ...
      int preemptingRoSpecId = (int) this.PreemptingROSpecID;
      try
      {
    ...
    public static PARAM_ROSpecEvent FromXmlNode(XmlNode node)
    {
      ...
      return new PARAM_ROSpecEvent()
      {
        EventType = (ENUM_ROSpecEventType) Enum.Parse(typeof (ENUM_ROSpecEventType), XmlUtil.GetNodeValue(node, "EventType")),
        ROSpecID = (uint) Util.ParseValueTypeFromString(XmlUtil.GetNodeValue(node, "ROSpecID"), "u32", ""),
        PreemptingROSpecID = (uint) Util.ParseValueTypeFromString(XmlUtil.GetNodeValue(node, "PreemptingROSpecID"), "u32", "")
      };
    }
    ...
    public override void AppendToBitArray(AutoGrowingBitArray bArr)
    {
      ...
      try
      {
        Util.AppendObjToBitArray((object) this.PreemptingROSpecID, (int) this.PreemptingROSpecID_len, bArr);
      }
      catch
      {
      }
