# LLRPSdk SDK 文档

## 1. 概述

### 1.1 简介

LLRPSdk 是一个基于 LLRP (Low Level Reader Protocol) 标准的 .NET SDK，用于控制符合 LLRP 协议的 RFID 读写器。该 SDK 提供了清晰的 API 接口，支持设备连接、标签盘点、内存操作和 GPIO 控制等功能。

### 1.2 命名空间

```csharp
using LLRPSdk;
```

### 1.3 快速开始

```csharp
// 创建读写器实例
var reader = new LlrpReader();

// 连接设备
reader.Connect("192.168.1.100");

// 应用默认配置（创建 ROSpec，必须在 Start 之前调用）
reader.ApplyDefaultSettings();

// 订阅标签报告事件
reader.TagsReported += (sender, args) =>
{
    foreach (var tag in args.TagReport.Tags)
    {
        Console.WriteLine($"EPC: {tag.Epc.ToHexString()}");
    }
};

// 开始盘点
reader.Start();

// 盘点 5 秒
Thread.Sleep(5000);

// 停止盘点
reader.Stop();

// 断开连接
reader.Disconnect();
```

> **重要**: 在调用 `Start()` 之前，必须先调用 `ApplyDefaultSettings()` 或 `ApplySettings()` 方法来创建 ROSpec（读取器操作规范），否则会抛出 "No ROSpec found on reader" 异常。

---

## 2. 核心类

### 2.1 LlrpReader 类

`LlrpReader` 类是控制 LLRP 读写器的主要入口点。

#### 构造函数

| 构造函数 | 描述 |
|-------------|-------------|
| `LlrpReader()` | 创建默认实例 |
| `LlrpReader(string address, string name)` | 创建具有指定地址和名称的实例 |

#### 属性

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `IsConnected` | bool | 指示是否已连接到读写器 |
| `Address` | string | 读写器 IP 地址 |
| `Name` | string | 读写器名称 |
| `ReaderCapabilities` | FeatureSet | 读写器能力信息 |
| `MessageTimeout` | int | 消息超时时间（毫秒），默认 5000 |
| `ConnectTimeout` | int | 连接超时时间（毫秒），默认 5000 |
| `LlrpMessageLogAsXml` | bool | LLRP 消息日志是否包含 XML 详情 |
| `Debug` | LlrpReaderDebug | 内置调试日志处理器 |

#### 方法

##### 连接管理

```csharp
// 同步连接
void Connect(string address);
void Connect(string address, int port);
void Connect(string address, bool useTLS);
void Connect(string address, int port, bool useTLS);
void Connect(string address, bool useTLS, TlsProtocols tlsProtocol);
void Connect(string address, int port, bool useTLS, TlsProtocols tlsProtocol);

// 异步连接
void ConnectAsync();
void ConnectAsync(string address);
void ConnectAsync(string address, int port);
void ConnectAsync(string address, bool useTLS);
void ConnectAsync(string address, int port, bool useTLS);
void ConnectAsync(string address, int port, bool useTLS, TlsProtocols tlsProtocol);

// 断开连接
void Disconnect();
```

##### 盘点操作

```csharp
// 开始盘点
void Start();

// 停止盘点
void Stop();
```

##### 状态和配置

```csharp
// 查询状态
Status QueryStatus();

// 查询配置
Settings QuerySettings();
Settings QueryDefaultSettings();

// 应用配置
void ApplySettings(Settings settings);
void ApplySettingsWithoutFactoryReset(Settings settings);
void ApplyDefaultSettings();

// 重置为出厂默认设置
void ResetToFactoryDefaultsOnly();
```

##### 设备能力

```csharp
// 查询设备能力
FeatureSet QueryFeatureSet();
```

##### GPIO 操作

```csharp
// 设置 GPO 状态
void SetGpo(ushort portNumber, bool state);
```

##### 标签操作序列

```csharp
// 添加操作序列
void AddOpSequence(TagOpSequence sequence);

// 删除操作序列
void DeleteOpSequence(uint sequenceId);
void DeleteAllOpSequences();

// 启用操作序列
void EnableOpSequence(uint sequenceId);

// 检查附加数据 AccessSpec 状态
bool? IsAttachedDataAccessSpecEnabled();

// 恢复附加数据 AccessSpec
void RestoreAttachedDataAccessSpec(bool wasEnabled);
```

##### 事件通知配置

```csharp
// 查询事件通知配置
List<ReaderEventNotificationState> QueryReaderEventNotifications();
```

##### 其他方法

```csharp
// 恢复事件和报告
void ResumeEventsAndReports();

// 查询缓冲区中的标签
void QueryTags();

// 构建 LLRP 消息
MSG_SET_READER_CONFIG BuildSetReaderConfigMessage(Settings settings);
MSG_ADD_ROSPEC BuildAddROSpecMessage(Settings settings);
```

#### 事件

| 事件 | 委托类型 | 描述 |
|-------|---------------|-------------|
| `TagsReported` | TagsReportedHandler | 标签报告事件 |
| `TagOpComplete` | TagOpCompleteHandler | 标签操作完成事件 |
| `GpiChanged` | GpiChangedHandler | GPI 状态变化事件 |
| `AntennaChanged` | AntennaEventHandler | 天线状态变化事件 |
| `AntennaStarted` | AntennaStartEventHandler | 天线启动事件 |
| `EndOfCycle` | EndOfCycleEventHandler | 盘点周期结束事件 |
| `ConnectionLost` | ConnectionLostHandler | 连接丢失事件 |
| `KeepaliveReceived` | KeepaliveHandler | 保活消息接收事件 |
| `ReaderStarted` | ReaderStartedEventHandler | 读写器启动事件 |
| `ReaderStopped` | ReaderStoppedEventHandler | 读写器停止事件 |
| `ReportBufferWarning` | ReportBufferWarningEventHandler | 报告缓冲区警告事件 |
| `ReportBufferOverflow` | ReportBufferOverflowEventHandler | 报告缓冲区溢出事件 |
| `ConnectAsyncComplete` | ConnectAsyncCompleteHandler | 异步连接完成事件 |
| `RawFrameReceived` | RawFrameReceivedHandler | 原始帧接收事件 |
| `RawFrameSent` | RawFrameSentHandler | 原始帧发送事件 |
| `LlrpMessageLogged` | LlrpMessageLoggedHandler | LLRP 消息日志事件 |
| `ErrorNotification` | ErrorNotificationHandler | 错误通知事件 |
| `DiagnosticsReported` | DiagnosticsReportedHandler | 诊断报告事件 |

---

### 2.2 Settings 类

`Settings` 类包含读写器配置参数。

#### 属性

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `AutoStart` | AutoStartConfig | 自动启动配置 |
| `AutoStop` | AutoStopConfig | 自动停止配置 |
| `RfMode` | uint? | RF 模式 |
| `Session` | ushort | 会话号 (0-3) |
| `TagPopulationEstimate` | ushort | 标签数量估计 |
| `Filters` | FilterSettings | 标签过滤设置 |
| `Report` | ReportConfig | 报告配置 |
| `Antennas` | AntennaConfigGroup | 天线配置集合 |
| `Gpis` | GpiConfigGroup | GPI 配置集合 |
| `Gpos` | GpoConfigGroup | GPO 配置集合 |
| `Keepalives` | KeepaliveConfig | 保活配置 |
| `HoldReportsOnDisconnect` | bool | 断开连接时保持报告 |
| `TxFrequenciesInMhz` | List\<double\> | 发射频率列表 |
| `ReducedPowerFrequenciesInMhz` | List\<double\> | 降低功率频率列表 |
| `StartOfAntennaEvent` | bool | 启用天线启动事件 |
| `EndOfCycleEvent` | bool | 启用周期结束事件 |
| `AttachedData` | AttachedDataConfig | 附加数据配置 |

#### 方法

```csharp
// 从 XML 字符串导入
static Settings FromXmlString(string xml);

// 从文件加载
static Settings Load(string path);
```

---

### 2.3 FeatureSet 类

`FeatureSet` 类包含读写器能力信息。

#### 属性

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `ModelNumber` | uint | 型号编号 |
| `ReaderModel` | string | 读写器型号名称 |
| `DeviceManufacturerNumber` | uint | 制造商编号 |
| `FirmwareVersion` | string | 固件版本 |
| `AntennaCount` | uint | 天线数量 |
| `GpiCount` | ushort | GPI 端口数量 |
| `GpoCount` | ushort | GPO 端口数量 |
| `MaxOperationSequences` | uint | 最大操作序列数 |
| `MaxOperationsPerSequence` | uint | 每个序列最大操作数 |
| `IsTagAccessAvailable` | bool | 标签访问支持 |
| `IsFilteringAvailable` | bool | 过滤支持 |
| `MaxTagSelectFiltersAllowed` | int | 最大标签选择过滤器数 |
| `IsMultiwordBlockWriteAvailable` | bool | 多字块写入支持 |
| `IsMultiwordBlockEraseAvailable` | bool | 多字块擦除支持 |
| `IsHoppingRegion` | bool | 是否跳频区域 |
| `CommunicationsStandard` | CommunicationsStandardType | 通信标准 |
| `CountryCode` | ushort | 国家代码 |
| `TxPowers` | List\<TxPowerTableEntry\> | 发射功率表 |
| `RxSensitivities` | List\<RxSensitivityTableEntry\> | 接收灵敏度表 |
| `ReaderMaxSensitivityActualDbm` | double | 读写器最大实际灵敏度 |
| `TxFrequencies` | List\<double\> | 发射频率列表 |
| `RfModes` | List\<uint?\> | RF 模式列表 |
| `RfModeDetails` | Dictionary\<uint, string\> | RF 模式详情 |

#### 方法

```csharp
// 从 XML 字符串导入
static FeatureSet FromXmlString(string xml);

// 从文件加载
static FeatureSet Load(string path);
```

---

### 2.4 Status 类

`Status` 类包含读写器当前状态信息。

#### 属性

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `IsConnected` | bool | 连接状态 |
| `IsSingulating` | bool | 是否正在盘点 |
| `Antennas` | AntennaStatusGroup | 天线状态集合 |
| `Gpis` | GpiStatusGroup | GPI 状态集合 |
| `GpoStates` | GpoStatusGroup | GPO 状态集合 |
| `ReaderIdentity` | object | 读写器标识 (MAC 地址) |

---

### 2.5 TagData 类

`TagData` 类表示标签数据。

#### 静态方法

```csharp
// 从十六进制字符串创建
static TagData FromHexString(string hex);

// 从字节数组创建
static TagData FromByteArray(byte[] bytes);

// 从字节列表创建
static TagData FromByteList(List<byte> data);

// 从无符号整数创建
static TagData FromUnsignedInt(uint data);

// 从字列表创建
static TagData FromWordList(List<ushort> data);

// 从字数组创建
static TagData FromWordArray(ushort[] data);

// 从单个字创建
static TagData FromWord(ushort data);
```

#### 属性

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `CountBytes` | int | 数据字节大小 |

#### 方法

```csharp
// 转换为字列表
List<ushort> ToList();

// 转换为十六进制字符串（带空格）
string ToHexWordString();

// 转换为十六进制字符串（无空格）
string ToHexString();

// 转换为无符号整数
uint ToUnsignedInt();
```

---

### 2.6 Tag 类

`Tag` 类包含特定标签的详细信息。

#### 属性

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `Epc` | TagData | EPC 数据 |
| `AntennaPortNumber` | ushort | 天线端口号 |
| `ChannelInMhz` | double | 频率 (MHz) |
| `FirstSeenTime` | Timestamp | 首次读取时间 |
| `LastSeenTime` | Timestamp | 最后读取时间 |
| `PeakRssi` | double | RSSI 值 |
| `TagSeenCount` | ushort | 读取次数 |
| `Crc` | ushort | CRC 值 |
| `PcBits` | ushort | PC 位 |
| `TxPower` | ushort | 发射功率 |
| `ReadOperationResults` | List\<TagReadOpResult\> | 读取操作结果列表 |

#### 状态标志属性

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `IsAntennaPortNumberPresent` | bool | 是否包含天线端口号数据 |
| `IsChannelInMhzPresent` | bool | 是否包含频率数据 |
| `IsFirstSeenTimePresent` | bool | 是否包含首次读取时间 |
| `IsLastSeenTimePresent` | bool | 是否包含最后读取时间 |
| `IsPeakRssiPresent` | bool | 是否包含 RSSI 数据 |
| `IsSeenCountPresent` | bool | 是否包含读取次数数据 |
| `IsCrcPresent` | bool | 是否包含 CRC 数据 |
| `IsPcBitsPresent` | bool | 是否包含 PC 位数据 |
| `IsTxPowerPresent` | bool | 是否包含发射功率数据 |

---

## 3. 连接管理

### 3.1 Connect 方法

**同步连接**:

```csharp
// 使用默认端口连接
reader.Connect("192.168.1.100");

// 使用指定端口连接
reader.Connect("192.168.1.100", 5084);

// 使用 TLS 加密连接
reader.Connect("192.168.1.100", true);

// 完整参数连接
reader.Connect("192.168.1.100", 5085, true, TlsProtocols.Tls12);
```

**参数说明**:

| 参数 | 类型 | 描述 |
|-----------|------|-------------|
| address | string | IP 地址或主机名 |
| port | int | 端口号（默认: 5084，加密: 5085） |
| useTLS | bool | 启用 TLS 加密 |
| tlsProtocol | TlsProtocols | TLS 协议版本 |

### 3.2 ConnectAsync 方法

**异步连接**:

```csharp
// 订阅连接完成事件
reader.ConnectAsyncComplete += (sender, result, errorMessage) =>
{
    if (result == ConnectAsyncResult.Success)
    {
        Console.WriteLine("连接成功");
    }
    else
    {
        Console.WriteLine($"连接失败: {errorMessage}");
    }
};

// 发起异步连接
reader.ConnectAsync("192.168.1.100");
```

### 3.3 Disconnect 方法

```csharp
// 断开连接
if (reader.IsConnected)
{
    reader.Disconnect();
}
```

### 3.4 连接事件

```csharp
// 连接丢失事件
reader.ConnectionLost += (sender) =>
{
    Console.WriteLine("连接丢失");
};
```

---

## 4. 盘点操作

### 4.1 Start 方法

```csharp
// 开始盘点
try
{
    reader.Start();
    Console.WriteLine("盘点已启动");
}
catch (LLRPSdkException ex)
{
    Console.WriteLine($"启动失败: {ex.Message}");
}
```

### 4.2 Stop 方法

```csharp
// 停止盘点
reader.Stop();
```

### 4.3 TagsReported 事件

```csharp
reader.TagsReported += (sender, args) =>
{
    var report = args.TagReport;

    foreach (var tag in report.Tags)
    {
        Console.WriteLine($"EPC: {tag.Epc.ToHexString()}");
        Console.WriteLine($"天线: {tag.AntennaPortNumber}");
        Console.WriteLine($"RSSI: {tag.PeakRssi} dBm");
        Console.WriteLine($"读取次数: {tag.TagSeenCount}");
        Console.WriteLine($"频率: {tag.ChannelInMhz} MHz");
    }
};
```

### 4.4 TagReport 类

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `Tags` | IList\<Tag\> | 标签列表 |

---

## 5. 标签操作

### 5.1 TagOpSequence 类

```csharp
var sequence = new TagOpSequence
{
    ExecutionCount = 1,              // 执行次数
    State = SequenceState.Active,    // 状态

    // 目标标签（可选，不设置则匹配所有标签）
    TargetTag = new TargetTag
    {
        MemoryBank = MemoryBank.Epc,
        Data = "E20034120123456789012345",
        BitPointer = 32
    },

    // 天线 ID（0 = 所有天线）
    AntennaId = 0
};

// 添加操作
sequence.Ops.Add(new TagReadOp { ... });

// 提交序列
reader.AddOpSequence(sequence);
```

#### TagOpSequence 属性

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `Id` | uint | 序列唯一 ID（自动分配） |
| `AntennaId` | ushort | 天线 ID |
| `State` | SequenceState | 序列状态 |
| `ExecutionCount` | ushort | 执行次数 |
| `TargetTag` | TargetTag | 目标标签 |
| `Ops` | List\<TagOp\> | 操作列表 |
| `BlockWriteEnabled` | bool | 是否启用块写入 |
| `BlockWriteWordCount` | ushort | 块写入字数 |
| `BlockWriteRetryCount` | ushort | 块写入重试次数 |
| `SequenceStopTrigger` | SequenceTriggerType | 序列停止触发器 |

### 5.2 TagReadOp 类

```csharp
var readOp = new TagReadOp
{
    MemoryBank = MemoryBank.User,    // 内存区
    WordPointer = 0,                 // 字指针
    WordCount = 8,                   // 字数
    AccessPassword = TagData.FromHexString("00000000")
};
```

#### TagReadOp 属性

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `Id` | ushort | 操作唯一 ID（自动分配） |
| `MemoryBank` | MemoryBank | 内存区 |
| `WordPointer` | ushort | 字指针 |
| `WordCount` | ushort | 读取字数 |
| `AccessPassword` | TagData | 访问密码 |

### 5.3 TagWriteOp 类

```csharp
var writeOp = new TagWriteOp
{
    MemoryBank = MemoryBank.Epc,
    WordPointer = 2,
    Data = TagData.FromHexString("1234567890ABCDEF"),
    AccessPassword = TagData.FromHexString("00000000")
};
```

#### TagWriteOp 属性

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `Id` | ushort | 操作唯一 ID（自动分配） |
| `MemoryBank` | MemoryBank | 内存区 |
| `WordPointer` | ushort | 字指针 |
| `Data` | TagData | 写入数据 |
| `AccessPassword` | TagData | 访问密码 |

### 5.4 TagLockOp 类

```csharp
var lockOp = new TagLockOp
{
    UserLockType = TagLockState.Lock,
    AccessPassword = TagData.FromHexString("00000000")
};
```

#### TagLockOp 属性

| 属性 | 类型 | 描述 |
|----------|------|-------------|
| `Id` | ushort | 操作唯一 ID |
| `KillPasswordLockType` | TagLockState | 销毁密码锁定类型 |
| `AccessPasswordLockType` | TagLockState | 访问密码锁定类型 |
| `EpcLockType` | TagLockState | EPC 锁定类型 |
| `TidLockType` | TagLockState | TID 锁定类型 |
| `UserLockType` | TagLockState | 用户内存锁定类型 |
| `AccessPassword` | TagData | 访问密码 |

### 5.5 TagKillOp 类

```csharp
var killOp = new TagKillOp
{
    KillPassword = TagData.FromHexString("12345678")
};
```

> **警告**: 销毁操作不可逆！

### 5.6 TagBlockEraseOp 类

```csharp
var blockEraseOp = new TagBlockEraseOp
{
    MemoryBank = MemoryBank.User,
    WordPointer = 0,
    WordCount = 4,
    AccessPassword = TagData.FromHexString("00000000")
};
```

### 5.7 TagOpComplete 事件

```csharp
reader.TagOpComplete += (sender, args) =>
{
    var report = args.TagOpReport;

    foreach (var result in report.Results)
    {
        Console.WriteLine($"EPC: {result.Tag.Epc.ToHexString()}");
        Console.WriteLine($"操作 ID: {result.OpId}");
        Console.WriteLine($"序列 ID: {result.SequenceId}");

        if (result is TagReadOpResult readResult)
        {
            Console.WriteLine($"读取数据: {readResult.Data?.ToHexString()}");
            Console.WriteLine($"结果状态: {readResult.Result}");
        }
        else if (result is TagWriteOpResult writeResult)
        {
            Console.WriteLine($"写入状态: {writeResult.Result}");
            Console.WriteLine($"写入字数: {writeResult.NumWordsWritten}");
        }
        else if (result is TagLockOpResult lockResult)
        {
            Console.WriteLine($"锁定状态: {lockResult.Result}");
        }
        else if (result is TagKillOpResult killResult)
        {
            Console.WriteLine($"销毁状态: {killResult.Result}");
        }
    }
};
```

---

## 6. 配置管理

### 6.1 QuerySettings 方法

```csharp
var settings = reader.QuerySettings();
Console.WriteLine($"会话: {settings.Session}");
Console.WriteLine($"标签数量估计: {settings.TagPopulationEstimate}");
```

### 6.2 ApplySettings 方法

```csharp
var settings = reader.QuerySettings();

// 修改配置
settings.Session = 2;
settings.TagPopulationEstimate = 32;

// 设置天线功率
foreach (var antenna in settings.Antennas)
{
    antenna.MaxTxPower = true;  // 使用最大功率
}

// 应用配置
reader.ApplySettings(settings);
```

### 6.3 天线配置

```csharp
var settings = reader.QuerySettings();

// 获取特定天线配置
var antenna1 = settings.Antennas[1];  // 端口号从 1 开始

antenna1.IsEnabled = true;           // 启用天线
antenna1.MaxTxPower = true;          // 最大发射功率
antenna1.MaxRxSensitivity = true;    // 最大接收灵敏度
antenna1.TxPowerInDbm = 30.0;        // 指定发射功率 (dBm)
antenna1.RxSensitivityInDbm = -70.0; // 指定接收灵敏度 (dBm)
```

### 6.4 报告配置

```csharp
var settings = reader.QueryDefaultSettings();

// 配置报告模式
settings.Report.Mode = ReportMode.Individual;  // 单独报告

// 配置报告内容
settings.Report.IncludeAntennaPortNumber = true;
settings.Report.IncludePeakRssi = true;
settings.Report.IncludeFirstSeenTime = true;
settings.Report.IncludeLastSeenTime = true;
settings.Report.IncludeSeenCount = true;
settings.Report.IncludePcBits = true;
settings.Report.IncludeCrc = true;

reader.ApplySettings(settings);
```

### 6.5 附加数据配置

```csharp
var settings = reader.QueryDefaultSettings();

// 配置附加数据（盘点时自动读取）
settings.AttachedData.Enabled = true;
settings.AttachedData.MemoryBank = MemoryBank.Tid;
settings.AttachedData.WordPointer = 0;
settings.AttachedData.WordCount = 6;
settings.AttachedData.AccessPassword = "00000000";

reader.ApplySettings(settings);
```

---

## 7. GPIO 操作

### 7.1 查询 GPI 状态

```csharp
var status = reader.QueryStatus();

foreach (GpiStatus gpi in status.Gpis)
{
    Console.WriteLine($"GPI {gpi.PortNumber}: {(gpi.State ? "高电平" : "低电平")}");
}
```

### 7.2 设置 GPO 状态

```csharp
// 设置单个 GPO
reader.SetGpo(1, true);  // 设置端口 1 为高电平
```

### 7.3 GpiChanged 事件

```csharp
reader.GpiChanged += (sender, args) =>
{
    Console.WriteLine($"GPI {args.PortNumber} 状态变化: {(args.State ? "高电平" : "低电平")}");
};
```

---

## 8. 枚举类型

### 8.1 MemoryBank

| 值 | 描述 |
|-------|-------------|
| `Reserved` | 保留区（密码） |
| `Epc` | EPC 内存区 |
| `Tid` | TID 内存区 |
| `User` | 用户内存区 |

### 8.2 TagLockState

| 值 | 描述 |
|-------|-------------|
| `None` | 无锁定 |
| `Unlock` | 解锁 |
| `Lock` | 锁定 |
| `Permaunlock` | 永久解锁 |
| `Permalock` | 永久锁定 |

### 8.3 AutoStartMode

| 值 | 描述 |
|-------|-------------|
| `None` | 手动启动 |
| `GpiTrigger` | GPI 触发 |
| `Periodic` | 周期性启动 |
| `UtcTimestamp` | 定时启动 |

### 8.4 AutoStopMode

| 值 | 描述 |
|-------|-------------|
| `None` | 手动停止 |
| `Duration` | 持续时间停止 |
| `GpiTrigger` | GPI 触发停止 |

### 8.5 TagFilterMode

| 值 | 描述 |
|-------|-------------|
| `None` | 无过滤 |
| `OnlyFilter1` | 仅过滤器 1 |
| `Filter1AndFilter2` | 过滤器 1 和 2 |
| `Filter1OrFilter2` | 过滤器 1 或 2 |
| `UseTagSelectFilters` | 使用标签选择过滤器 |

### 8.6 ReportMode

| 值 | 描述 |
|-------|-------------|
| `Individual` | 单独报告 |
| `BatchAfterStop` | 停止后批量报告 |
| `WaitForQuery` | 等待查询 |

### 8.7 SequenceState

| 值 | 描述 |
|-------|-------------|
| `Disabled` | 禁用 |
| `Active` | 激活 |

### 8.8 SequenceTriggerType

| 值 | 描述 |
|-------|-------------|
| `None` | 无触发 |
| `ExecutionCount` | 执行次数 |

### 8.9 TlsProtocols

| 值 | 描述 |
|-------|-------------|
| `OsDefault` | 操作系统默认 |
| `Tls12` | TLS 1.2 |
| `Tls13` | TLS 1.3 |

---

## 9. 异常处理

### 9.1 LLRPSdkException 类

```csharp
try
{
    reader.Start();
}
catch (LLRPSdkException ex)
{
    Console.WriteLine($"LLRP 错误: {ex.Message}");
}
```

**常见异常**:

| 消息 | 原因 |
|---------|---------|
| You must connect to the reader before starting it. | 未连接时调用 Start |
| Timeout. | 连接超时 |
| A reader initiated connection already exists. | 已存在另一个连接 |
| No ROSpec found on reader. | 设备上无 ROSpec |
| The Address property must be set before calling Connect. | 未设置地址即调用连接 |

---

## 10. 日志与调试

### 10.1 LLRP 消息日志

```csharp
// 启用 XML 详细日志
reader.LlrpMessageLogAsXml = true;

// 订阅 LLRP 消息日志事件
reader.LlrpMessageLogged += (sender, message) =>
{
    Console.WriteLine(message);
};
```

### 10.2 原始帧日志

```csharp
// 订阅原始帧接收事件
reader.RawFrameReceived += (sender, raw) =>
{
    Console.WriteLine($"接收: {raw.Length} 字节");
};

// 订阅原始帧发送事件
reader.RawFrameSent += (sender, raw) =>
{
    Console.WriteLine($"发送: {raw.Length} 字节");
};
```

### 10.3 调试处理器

```csharp
// 使用内置调试处理器
reader.Debug.EnableLogging = true;
```

---

## 11. 完整示例

### 11.1 基本盘点示例

```csharp
using LLRPSdk;

class Program
{
    static void Main(string[] args)
    {
        var reader = new LlrpReader();
        
        try
        {
            // 连接读写器
            Console.WriteLine("正在连接...");
            reader.Connect("192.168.1.100");
            Console.WriteLine("连接成功");
            
            // 显示读写器信息
            var caps = reader.ReaderCapabilities;
            Console.WriteLine($"型号: {caps.ReaderModel}");
            Console.WriteLine($"固件: {caps.FirmwareVersion}");
            Console.WriteLine($"天线数: {caps.AntennaCount}");
            
            // 应用默认配置（创建 ROSpec，必须在 Start 之前调用）
            reader.ApplyDefaultSettings();
            
            // 订阅标签报告
            reader.TagsReported += (sender, report) =>
            {
                foreach (var tag in report.Tags)
                {
                    Console.WriteLine($"EPC: {tag.Epc.ToHexString()}");
                }
            };
            
            // 开始盘点
            Console.WriteLine("开始盘点...");
            reader.Start();
            
            // 盘点 10 秒
            Thread.Sleep(10000);
            
            // 停止盘点
            reader.Stop();
            Console.WriteLine("盘点结束");
        }
        catch (LLRPSdkException ex)
        {
            Console.WriteLine($"错误: {ex.Message}");
        }
        finally
        {
            // 断开连接
            if (reader.IsConnected)
            {
                reader.Disconnect();
                Console.WriteLine("已断开连接");
            }
        }
    }
}
```

### 11.2 标签写入示例

```csharp
using LLRPSdk;

class Program
{
    static void Main(string[] args)
    {
        var reader = new LlrpReader();
        
        try
        {
            reader.Connect("192.168.1.100");
            
            // 订阅操作完成事件
            reader.TagOpComplete += (sender, report) =>
            {
                foreach (var result in report.Results)
                {
                    if (result is TagWriteOpResult writeResult)
                    {
                        Console.WriteLine($"写入结果: {writeResult.Result}");
                        Console.WriteLine($"写入字数: {writeResult.NumWordsWritten}");
                    }
                }
            };
            
            // 创建写入序列
            var sequence = new TagOpSequence
            {
                ExecutionCount = 1,
                State = SequenceState.Active,
                AntennaId = 0  // 所有天线
            };
            
            // 添加写入操作
            var writeOp = new TagWriteOp
            {
                MemoryBank = MemoryBank.Epc,
                WordPointer = 2,
                Data = TagData.FromHexString("E20034120123456789012345"),
                AccessPassword = TagData.FromHexString("00000000")
            };
            sequence.Ops.Add(writeOp);
            
            // 提交序列
            reader.AddOpSequence(sequence);
            
            // 开始盘点（触发写入操作）
            reader.Start();
            Thread.Sleep(5000);
            reader.Stop();
        }
        catch (LLRPSdkException ex)
        {
            Console.WriteLine($"错误: {ex.Message}");
        }
        finally
        {
            if (reader.IsConnected)
                reader.Disconnect();
        }
    }
}
```

### 11.3 读取用户内存示例

```csharp
using LLRPSdk;

class Program
{
    static void Main(string[] args)
    {
        var reader = new LlrpReader();
        
        try
        {
            reader.Connect("192.168.1.100");
            
            // 订阅操作完成事件
            reader.TagOpComplete += (sender, report) =>
            {
                foreach (var result in report.Results)
                {
                    if (result is TagReadOpResult readResult)
                    {
                        Console.WriteLine($"读取结果: {readResult.Result}");
                        if (readResult.Data != null)
                        {
                            Console.WriteLine($"读取数据: {readResult.Data.ToHexString()}");
                        }
                    }
                }
            };
            
            // 创建读取序列
            var sequence = new TagOpSequence
            {
                ExecutionCount = 1,
                State = SequenceState.Active,
                AntennaId = 0
            };
            
            // 添加读取操作
            var readOp = new TagReadOp
            {
                MemoryBank = MemoryBank.User,
                WordPointer = 0,
                WordCount = 4,
                AccessPassword = TagData.FromHexString("00000000")
            };
            sequence.Ops.Add(readOp);
            
            reader.AddOpSequence(sequence);
            
            reader.Start();
            Thread.Sleep(5000);
            reader.Stop();
        }
        catch (LLRPSdkException ex)
        {
            Console.WriteLine($"错误: {ex.Message}");
        }
        finally
        {
            if (reader.IsConnected)
                reader.Disconnect();
        }
    }
}
```

---

## 附录

### A. 端口参考

| 端口 | 描述 |
|------|-------------|
| 5084 | LLRP 默认端口（未加密） |
| 5085 | LLRP 加密端口（TLS） |

### B. 制造商编号

| 编号 | 制造商 |
|--------|-------------|
| 25882 | Impinj |
| 161 | Motorola |
| 10642 | Zebra |
| 47706 | Silion |
| 57690 | Seuic |

### C. 支持的读写器型号

**Impinj 型号**:
- Speedway R420
- Speedway R220
- Speedway R1000
- R700
- R720
- xPortal
- xArray

**Zebra/Motorola 型号**:
- FX9600
- FX7500
- MC3390xR
- MC3380xR

### D. 参考资料

- LLRP 规范: [EPCglobal LLRP 1.1](https://www.gs1.org/standards/epcrfid-epcglobal/llrp-1-1)
- Impinj 开发者中心: [Impinj Developer Hub](https://developer.impinj.com/)

---

*文档版本: 1.0*
*最后更新: 2024*
*适用于 LLRPSdk .NET*
